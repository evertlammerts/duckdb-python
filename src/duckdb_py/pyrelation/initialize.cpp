...
static void InitializeConsumers(py::class_<DuckDBPyRelation> &m) {
	...
	m.def("fetchone", &DuckDBPyRelation::FetchOne, "Execute and fetch a single row as a tuple")
	    ...
	    .def("arrow_table", &DuckDBPyRelation::ToArrowTable, "Execute and fetch all rows as an Arrow Table",
	         py::arg("batch_size") = 1000000)
	    .def("arrow_reader", &DuckDBPyRelation::ToRecordBatch,
	    ...
	    .def(
	        ...
	        [](pybind11::object &self, idx_t batch_size) {
		        PyErr_WarnEx(PyExc_DeprecationWarning, "arrow() is deprecated, use arrow_reader() instead.", 0);
		        return self.attr("arrow_reader")(batch_size);
	        },
	        "Execute and return an Arrow Record Batch Reader that yields all rows", py::arg("batch_size") = 1000000)
	    .def(
	        "fetch_arrow_table",
	        [](pybind11::object &self, idx_t batch_size) {
		        PyErr_WarnEx(PyExc_DeprecationWarning, "fetch_arrow_table() is deprecated, use arrow_table() instead.",
		                     0);
		        return self.attr("arrow_table")(batch_size);
	        },
	        "Execute and fetch all rows as an Arrow Table", py::arg("batch_size") = 1000000)
	    .def(
	        "to_arrow_table",
	        [](pybind11::object &self, idx_t batch_size) {
		        PyErr_WarnEx(PyExc_DeprecationWarning, "to_arrow_table() is deprecated, use arrow_table() instead.", 0);
		        return self.attr("arrow_table")(batch_size);
	        },
	        "Execute and fetch all rows as an Arrow Table", py::arg("batch_size") = 1000000)
	...
	m.def(
	     ...
	     [](pybind11::object &self, idx_t rows_per_batch) {
		     PyErr_WarnEx(PyExc_DeprecationWarning, "fetch_record_batch() is deprecated, use arrow_reader() instead.",
		                  0);
		     return self.attr("arrow_reader")(rows_per_batch);
	     },
	    ...
	    .def(
	        ...
	        [](pybind11::object &self, idx_t batch_size) {
		        PyErr_WarnEx(PyExc_DeprecationWarning,
		                     "fetch_arrow_reader() is deprecated, use arrow_reader() instead.", 0);
		        return self.attr("arrow_reader")(batch_size);
	        },
	        "Execute and return an Arrow Record Batch Reader that yields all rows", py::arg("batch_size") = 1000000);
}
