...
void DuckDBPyConnection::DetectEnvironment() {
	...
	auto main_module = py::module_::import_("__main__");
...
void DuckDBPyConnection::Initialize(py::handle &m) {
	...
	connection_module.def("__enter__", &DuckDBPyConnection::Enter)
	    .def(
	        ...
	        py::arg("exc_type").none(), py::arg("exc").none(), py::arg("traceback").none());
...
vector<unique_ptr<SQLStatement>> DuckDBPyConnection::GetStatements(const py::object &query) {
	...
	if (py::isinstance<py::str>(query)) {
		auto &connection = con.GetConnection();
		auto sql_query = py::cast<std::string>(py::str(query));
...
static void ParseMultiFileOptions(ClientContext &context, named_parameter_map_t &options,
	...
	if (!py::none().is(hive_partitioning)) {
		if (!py::isinstance<py::bool_>(hive_partitioning)) {
			string actual_type = py::cast<std::string>(py::str((hive_partitioning).type()));
	...
	if (!py::none().is(union_by_name)) {
		if (!py::isinstance<py::bool_>(union_by_name)) {
			string actual_type = py::cast<std::string>(py::str((union_by_name).type()));
	...
	if (!py::none().is(hive_types_autocast)) {
		if (!py::isinstance<py::bool_>(hive_types_autocast)) {
			string actual_type = py::cast<std::string>(py::str((hive_types_autocast).type()));
...
std::unique_ptr<DuckDBPyRelation> DuckDBPyConnection::ReadJSON(
	...
	if (!py::none().is(records)) {
		if (!py::isinstance<py::str>(records)) {
			string actual_type = py::cast<std::string>(py::str((records).type()));
	...
	if (!py::none().is(date_format)) {
		...
		auto date_format_option = py::cast<std::string>(py::str(date_format_s));
		options["date_format"] = Value(date_format_option);
	}

	if (!py::none().is(timestamp_format)) {
		if (!py::isinstance<py::str>(timestamp_format)) {
			string actual_type = py::cast<std::string>(py::str((timestamp_format).type()));
		...
		auto timestamp_format_option = py::cast<std::string>(py::str(timestamp_format_s));
		options["timestamp_format"] = Value(timestamp_format_option);
	}

	if (!py::none().is(compression)) {
		if (!py::isinstance<py::str>(compression)) {
			string actual_type = py::cast<std::string>(py::str((compression).type()));
		...
		auto compression_option = py::cast<std::string>(py::str(compression_s));
	...
	if (!py::none().is(maximum_object_size)) {
		if (!py::isinstance<py::int_>(maximum_object_size)) {
			string actual_type = py::cast<std::string>(py::str((maximum_object_size).type()));
	...
	if (!py::none().is(ignore_errors)) {
		if (!py::isinstance<py::bool_>(ignore_errors)) {
			string actual_type = py::cast<std::string>(py::str((ignore_errors).type()));
	...
	if (!py::none().is(convert_strings_to_integers)) {
		if (!py::isinstance<py::bool_>(convert_strings_to_integers)) {
			string actual_type = py::cast<std::string>(py::str((convert_strings_to_integers).type()));
	...
	if (!py::none().is(field_appearance_threshold)) {
		if (!py::isinstance<py::float_>(field_appearance_threshold)) {
			string actual_type = py::cast<std::string>(py::str((field_appearance_threshold).type()));
	...
	if (!py::none().is(map_inference_threshold)) {
		if (!py::isinstance<py::int_>(map_inference_threshold)) {
			string actual_type = py::cast<std::string>(py::str((map_inference_threshold).type()));
	...
	if (!py::none().is(maximum_sample_files)) {
		if (!py::isinstance<py::int_>(maximum_sample_files)) {
			string actual_type = py::cast<std::string>(py::str((maximum_sample_files).type()));
...
std::unique_ptr<DuckDBPyRelation> DuckDBPyConnection::ReadCSV(const py::object &name_p, py::kwargs &kwargs) {
	...
	if (!py::none().is(names_p)) {
		...
		for (auto elem : names_list) {
			...
			names.push_back(Value(py::cast<std::string>(py::str(elem))));
		}
	...
	if (!py::none().is(na_values)) {
		...
		if (!py::isinstance<py::str>(na_values) && !duckdb::PyUtil::IsListLike(na_values)) {
			...
			for (auto elem : null_list) {
				...
				null_values.push_back(Value(py::cast<std::string>(py::str(elem))));
			}
		}
	...
	if (!py::none().is(lineterminator)) {
		...
		if (!py::try_cast<PythonCSVLineTerminator::Type>(lineterminator, new_line_type)) {
			string actual_type = py::cast<std::string>(py::str((lineterminator).type()));
	...
	if (!py::none().is(max_line_size)) {
		if (!py::isinstance<py::str>(max_line_size) && !py::isinstance<py::int_>(max_line_size)) {
			string actual_type = py::cast<std::string>(py::str((max_line_size).type()));
	...
	if (!py::none().is(auto_type_candidates)) {
		if (!py::isinstance<py::list>(auto_type_candidates)) {
			string actual_type = py::cast<std::string>(py::str((auto_type_candidates).type()));
	...
	if (!py::none().is(store_rejects)) {
		if (!py::isinstance<py::bool_>(store_rejects)) {
			string actual_type = py::cast<std::string>(py::str((store_rejects).type()));
	...
	if (!py::none().is(rejects_table)) {
		if (!py::isinstance<py::str>(rejects_table)) {
			string actual_type = py::cast<std::string>(py::str((rejects_table).type()));
	...
	if (!py::none().is(rejects_scan)) {
		if (!py::isinstance<py::str>(rejects_scan)) {
			string actual_type = py::cast<std::string>(py::str((rejects_scan).type()));
	...
	if (!py::none().is(rejects_limit)) {
		if (!py::isinstance<py::int_>(rejects_limit)) {
			string actual_type = py::cast<std::string>(py::str((rejects_limit).type()));
	...
	if (!py::none().is(force_not_null)) {
		if (!py::isinstance<py::list>(force_not_null)) {
			string actual_type = py::cast<std::string>(py::str((force_not_null).type()));
	...
	if (!py::none().is(buffer_size)) {
		if (!py::isinstance<py::int_>(buffer_size)) {
			string actual_type = py::cast<std::string>(py::str((buffer_size).type()));
	...
	if (!py::none().is(decimal)) {
		if (!py::isinstance<py::str>(decimal)) {
			string actual_type = py::cast<std::string>(py::str((decimal).type()));
	...
	if (!py::none().is(allow_quoted_nulls)) {
		if (!py::isinstance<py::bool_>(allow_quoted_nulls)) {
			string actual_type = py::cast<std::string>(py::str((allow_quoted_nulls).type()));
...
static string GetPathString(const py::object &path) {
	...
	string actual_type = py::cast<std::string>(py::str((path).type()));
...
bool IsValidNumpyDimensions(const py::handle &object, int &dim) {
	...
	int cur_dim = py::cast<int>((shape.attr("__getitem__")(0)));
