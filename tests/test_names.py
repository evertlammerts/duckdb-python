...
@pytest.fixture
def dotted() -> duckdb.frame.Frame:
    return values([(21, 1)], columns=["hello.world", "id"])


class TestAStringIsOneName:
    def test_every_verb_takes_a_dotted_name_whole(
        self, con: duckdb.frame.Connection, dotted: duckdb.frame.Frame
    ) -> None:
    ...
    def test_column_names_round_trip(self, con: duckdb.frame.Connection, dotted: duckdb.frame.Frame) -> None:
        assert dotted.select(*dotted.columns(con)).rows(con) == dotted.rows(con)

    def test_a_struct_named_like_a_prefix_does_not_shadow(self, con: duckdb.frame.Connection) -> None:
    ...
    def test_using_a_dotted_key(self, con: duckdb.frame.Connection) -> None:
        left = values([(1, 5)], columns=["k.1", "x"])
        right = values([(1, 9)], columns=["k.1", "y"])
        assert left.join(right, on="k.1").rows(con) == [(1, 5, 9)]

    def test_drop_takes_any_name_through_exclude(
        self, con: duckdb.frame.Connection, dotted: duckdb.frame.Frame
    ) -> None:
    ...
    def test_rename_takes_any_name_through_rename(
        self, con: duckdb.frame.Connection, dotted: duckdb.frame.Frame
    ) -> None:
    ...
    def test_star_exclude_and_rename_take_any_name(
        self, con: duckdb.frame.Connection, dotted: duckdb.frame.Frame
    ) -> None:
...
class TestQualifiedNamesAreTuples:
    def test_a_tuple_qualifies_a_table(self, con: duckdb.frame.Connection) -> None:
        assert table(("s", "t")).rows(con) == [(1,)]
        assert table(("s", "t")).render() == 'SELECT * FROM "s"."t"'
        assert table(("memory", "s", "t")).rows(con) == [(1,)]

    def test_a_dotted_string_is_one_table(self, con: duckdb.frame.Connection) -> None:
        assert table("a.b").rows(con) == [(2,)]
        with pytest.raises(exceptions.CatalogError, match=r"s\.t"):
            table("s.t").rows(con)

    def test_a_file_path_is_one_name(self, con: duckdb.frame.Connection, tmp_path: Path) -> None:
    ...
    def test_create_and_insert_into_take_tuples(self, con: duckdb.frame.Connection) -> None:
    ...
    def test_create_macro_takes_a_tuple(self, con: duckdb.frame.Connection) -> None:
        con.create_macro(("s", "thrice"), ["x"], col("x") * 3)
        assert sql("SELECT s.thrice(4)").rows(con) == [(12,)]

    def test_function_names_are_written_as_the_engine_writes_identifiers(self, con: duckdb.frame.Connection) -> None:
    ...
    def test_a_keyword_as_a_function_name_is_quoted(self, con: duckdb.frame.Connection) -> None:
    ...
    def test_coalesce_is_syntax_not_a_function(self, con: duckdb.frame.Connection) -> None:
    ...
    def test_table_functions_follow_the_same_rule(self, con: duckdb.frame.Connection) -> None:
    ...
    def test_the_keyword_module_is_current(self, con: duckdb.frame.Connection) -> None:
...
class TestBracketsReachIntoValues:
    def test_struct_list_and_map(self, con: duckdb.frame.Connection) -> None:
    ...
    def test_a_plan_bracket_still_narrows_the_plan(
        ...
        assert isinstance(narrowed, duckdb.frame.Frame)
...
class TestJoinSidesAreTheConditionsArguments:
    def test_the_condition_is_a_function_of_both_sides(self, con: duckdb.frame.Connection) -> None:
    ...
    def test_sides_take_dotted_names_and_fields(self, con: duckdb.frame.Connection) -> None:
    ...
    def test_the_condition_runs_once_on_side_objects(self, con: duckdb.frame.Connection) -> None:
        calls: list[tuple[type, type]] = []

        def same_id(left: Side, right: Side) -> duckdb.frame.Expr:
    ...
    def test_a_suffixed_join_on_a_dotted_key(self, con: duckdb.frame.Connection) -> None:
        left = values([(1, 5)], columns=["k.1", "x"])
    ...
    def test_a_value_from_outside_binds(self, con: duckdb.frame.Connection) -> None:
        floor = 5
    ...
    def test_a_dotted_side_name_is_a_column_now(self, con: duckdb.frame.Connection) -> None:
        rows = values([(1,)], columns=["id"])
        with pytest.raises(exceptions.Error, match=r"l\.id"):
            rows.join(rows, on=col("l.id") == col("r.id"), suffix="_r").rows(con)

    def test_raw_sql_names_the_sides_itself(self, con: duckdb.frame.Connection) -> None:
        rows = values([(1,)], columns=["id"])
...
class TestNamesCompareAsTheEngineDoes:
    def test_case_is_not_a_difference(self, con: duckdb.frame.Connection) -> None:
    ...
    def test_two_spellings_of_one_name_are_refused(self, con: duckdb.frame.Connection) -> None:
...
class TestFloatsAreDoubles:
    def test_a_float_is_a_double_inline_and_bound(self, con: duckdb.frame.Connection) -> None:
...
class TestTheParameterizedForm:
    def test_render_parameterized_is_what_executes(self, con: duckdb.frame.Connection) -> None:
