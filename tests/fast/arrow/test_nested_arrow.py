...
def compare_results(duckdb_cursor, query):
    ...
    produced_arrow = duckdb_cursor.query(query).arrow_table()
...
def arrow_to_pandas(duckdb_cursor, query):
    return duckdb_cursor.query(query).arrow_table().to_pandas()["a"].values.tolist()
...
class TestArrowNested:
    def test_lists_basic(self, duckdb_cursor):
        # Test Constant List
        query = duckdb_cursor.query("SELECT a from (select list_value(3,5,10) as a) as t").arrow_table()["a"].to_numpy()
        assert query[0][0] == 3
        assert query[0][1] == 5
        assert query[0][2] == 10

        # Empty List
        query = duckdb_cursor.query("SELECT a from (select list_value() as a) as t").arrow_table()["a"].to_numpy()
        assert len(query[0]) == 0

        # Test Constant List With Null
        query = duckdb_cursor.query("SELECT a from (select list_value(3,NULL) as a) as t").arrow_table()["a"].to_numpy()
        assert query[0][0] == 3
