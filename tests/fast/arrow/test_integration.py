...
class TestArrowIntegration:
    def test_parquet_roundtrip(self, duckdb_cursor):
        ...
        rel_from_arrow = duckdb.arrow(userdata_parquet_table).project(cols).arrow_table()
        rel_from_arrow.validate(full=True)

        rel_from_duckdb = duckdb_cursor.from_parquet(parquet_filename).project(cols).arrow_table()
        ...
        for i in [7, 51, 99, 100, 101, 500, 1000, 2000]:
            ...
            rel_from_arrow2 = duckdb.arrow(userdata_parquet_table2).project(cols).arrow_table()
    ...
    def test_unsigned_roundtrip(self, duckdb_cursor):
        ...
        rel_from_arrow = duckdb.arrow(unsigned_parquet_table).project(cols).arrow_table()
        rel_from_arrow.validate(full=True)
        ...
        arrow_result = duckdb_cursor.arrow_table()
        arrow_result.validate(full=True)
        arrow_result.combine_chunks()
        arrow_result.validate(full=True)

        round_tripping = duckdb.from_arrow(arrow_result).arrow_table()
    ...
    def test_decimals_roundtrip(self, duckdb_cursor):
        ...
        duck_from_arrow = duckdb_cursor.from_arrow(duck_tbl.arrow_table())
    ...
    def test_intervals_roundtrip(self, duckdb_cursor):
        ...
        duck_arrow_tbl = duckdb_cursor.table("intervaltbl").arrow_table()["a"]
        ...
        duck_tbl_arrow = duckdb_cursor.table("test").arrow_table()["a"]
    ...
    def test_null_intervals_roundtrip(self, duckdb_cursor):
        ...
        duckdb_tbl_arrow = duckdb_cursor.table("intervalnulltbl").arrow_table()["a"]
    ...
    def test_nested_interval_roundtrip(self, duckdb_cursor):
        ...
        duckdb_tbl_arrow = duckdb_cursor.table("dictionarytbl").arrow_table()["a"]
        ...
        from_arrow = duckdb_cursor.from_arrow(duckdb_cursor.sql(query).arrow_table()).fetchall()
    ...
    def test_min_max_interval_roundtrip(self, duckdb_cursor):
        ...
        duck_arrow_tbl = duckdb_cursor.table("intervalminmaxtbl").arrow_table()["a"]
