...
class TestCanonicalExtensionTypes:
    def test_uuid(self):
        ...
        duck_arrow = duckdb_cursor.execute("FROM arrow_table").arrow_table()
    ...
    def test_uuid_from_duck(self):
        ...
        arrow_table = duckdb_cursor.execute("select uuid from test_all_types()").arrow_table()
        ...
        arrow_table = duckdb_cursor.execute("select '00000000-0000-0000-0000-000000000100'::UUID as uuid").arrow_table()
    ...
    def test_uuid_no_def(self):
        ...
        res_arrow = duckdb_cursor.execute("select uuid from test_all_types()").arrow_table()
    ...
    def test_uuid_no_def_stream(self):
        ...
        res_arrow = duckdb_cursor.execute("select uuid from test_all_types()").arrow_reader()
    ...
    def test_hugeint(self):
        ...
        assert con.execute("FROM arrow_table").arrow_table().equals(arrow_table)

        con.execute("SET arrow_lossless_conversion = false")

        assert not con.execute("FROM arrow_table").arrow_table().equals(arrow_table)
    ...
    def test_bit(self):
        ...
        res_blob = con.execute("SELECT '0101011'::BIT str FROM range(5) tbl(i)").arrow_table()

        con.execute("SET arrow_lossless_conversion = true")

        res_bit = con.execute("SELECT '0101011'::BIT str FROM range(5) tbl(i)").arrow_table()
    ...
    def test_timetz(self):
        ...
        res_time = con.execute("SELECT '02:30:00+04'::TIMETZ str FROM range(1) tbl(i)").arrow_table()

        con.execute("SET arrow_lossless_conversion = true")

        res_tz = con.execute("SELECT '02:30:00+04'::TIMETZ str FROM range(1) tbl(i)").arrow_table()
    ...
    def test_nested_types_with_extensions(self):
        ...
        arrow_table = duckdb_cursor.execute("select map {uuid(): 1::uhugeint, uuid(): 2::uhugeint} as li").arrow_table()
    ...
    def test_boolean(self):
        ...
        result_table = con.execute("FROM arrow_table").arrow_table()
    ...
    def test_accept_malformed_complex_json(self, duckdb_cursor):
        ...
        tbl = duckdb_cursor.sql("""SELECT geometry as wkt FROM geo_table;""").arrow_table()
