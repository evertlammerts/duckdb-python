...
class TestArrowLargeOffsets:
    ...
    def test_large_lists(self, duckdb_cursor):
        ...
        with pytest.raises(
            ...
            duckdb_cursor.sql("SELECT col FROM tbl").arrow_table()
        ...
        res2 = duckdb_cursor.sql("SELECT col FROM tbl2").arrow_table()
        res2.validate()
    ...
    def test_large_maps(self, duckdb_cursor):
        ...
        with pytest.raises(
            ...
            duckdb_cursor.sql("select map(col, col) from tbl").arrow_table()

        duckdb_cursor.sql("set arrow_large_buffer_size = true")
        arrow_map_large = duckdb_cursor.sql("select map(col, col) from tbl").arrow_table()
