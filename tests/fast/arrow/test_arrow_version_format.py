...
class TestArrowDecimalTypes:
    def test_decimal_v1_5(self, duckdb_cursor):
        ...
        col_type = duckdb_cursor.execute("FROM decimal_32").arrow_table().schema.field("data").type
        ...
        col_type = duckdb_cursor.execute("FROM decimal_64").arrow_table().schema.field("data").type
        assert col_type.bit_width == 64
        assert pa.types.is_decimal(col_type)
        for version in ["1.0", "1.1", "1.2", "1.3", "1.4"]:
            duckdb_cursor.execute(f"SET arrow_output_version = {version}")
            result = duckdb_cursor.execute("FROM decimal_32").arrow_table()
            ...
            result = duckdb_cursor.execute("FROM decimal_64").arrow_table()
            col_type = result.schema.field("data").type
            assert col_type.bit_width == 128
            assert pa.types.is_decimal(col_type)
            assert result.to_pydict() == {
    ...
    def test_view_v1_4(self, duckdb_cursor):
        ...
        col_type = duckdb_cursor.execute("SELECT 'string' as data ").arrow_table().schema.field("data").type
