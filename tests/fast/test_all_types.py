...
class TestAllTypes:
    ...
    def test_arrow(self, cur_type):
        ...
        conn = duckdb.connect()
        if cur_type in replacement_values:
            arrow_table = conn.execute("select " + replacement_values[cur_type]).arrow_table()
        else:
            arrow_table = conn.execute(f'select "{cur_type}" from test_all_types()').arrow_table()
        if cur_type in enum_types:
            round_trip_arrow_table = conn.execute("select * from arrow_table").arrow_table()
