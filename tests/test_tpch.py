...
import duckdb
from duckdb.frame import Expr, col, fn, sql_expr, when
...
def answer(connection: duckdb.frame.Connection, name: str) -> list[tuple[object, ...]]:
    """What the original query returns, for the verb version to match."""
    return duckdb.frame.sql(query_text(Path(__file__).parent / "tpch" / f"{name}.sql")).rows(connection)
...
def test_the_sql_bridge_carries_every_query(tpch: duckdb.frame.Connection, path: Path) -> None:
    """`sql()` accepts any query DuckDB accepts, which is what makes an unexpressible query harmless."""
    rows = duckdb.frame.sql(query_text(path)).rows(tpch)
    assert isinstance(rows, list)


@pytest.mark.parametrize("path", QUERIES, ids=lambda p: p.stem)
def test_schema_is_available_without_running(tpch: duckdb.frame.Connection, path: Path) -> None:
    """Every query's column names and types are available without running it."""
    frame = duckdb.frame.sql(query_text(path))
...
class TestExpressedDirectly:
    ...
    def test_q01(self, tpch: duckdb.frame.Connection) -> None:
        disc = revenue()
        actual = (
            duckdb.frame.table("lineitem")
    ...
    def test_q03(self, tpch: duckdb.frame.Connection) -> None:
        customer = duckdb.frame.table("customer").filter(col("c_mktsegment") == "BUILDING")
        orders = duckdb.frame.table("orders").filter(col("o_orderdate") < date("1995-03-15"))
        lineitem = duckdb.frame.table("lineitem").filter(col("l_shipdate") > date("1995-03-15"))
        actual = (
    ...
    def test_q05(self, tpch: duckdb.frame.Connection) -> None:
        orders = duckdb.frame.table("orders").filter(
            (col("o_orderdate") >= date("1994-01-01")) & (col("o_orderdate") < date("1995-01-01"))
        )
        actual = (
            duckdb.frame.table("customer")
            .join(orders, on=lambda left, right: left["c_custkey"] == right["o_custkey"])
            .join(duckdb.frame.table("lineitem"), on=lambda left, right: left["o_orderkey"] == right["l_orderkey"])
            .join(
                duckdb.frame.table("supplier"),
                on=lambda left, right: (
                    (left["l_suppkey"] == right["s_suppkey"]) & (left["c_nationkey"] == right["s_nationkey"])
                ),
            )
            .join(duckdb.frame.table("nation"), on=lambda left, right: left["s_nationkey"] == right["n_nationkey"])
            .join(
                duckdb.frame.table("region").filter(col("r_name") == "ASIA"),
    ...
    def test_q06(self, tpch: duckdb.frame.Connection) -> None:
    ...
    def test_q07(self, tpch: duckdb.frame.Connection) -> None:
        # `nation` is joined twice, and the second join suffixes its side, so it arrives as n_name_cust.
        actual = (
            duckdb.frame.table("supplier")
            .join(
                duckdb.frame.table("lineitem").filter(
                    col("l_shipdate").between(date("1995-01-01"), date("1996-12-31"))
                ),
                on=lambda left, right: left["s_suppkey"] == right["l_suppkey"],
            )
            .join(duckdb.frame.table("orders"), on=lambda left, right: left["l_orderkey"] == right["o_orderkey"])
            .join(duckdb.frame.table("customer"), on=lambda left, right: left["o_custkey"] == right["c_custkey"])
            .join(duckdb.frame.table("nation"), on=lambda left, right: left["s_nationkey"] == right["n_nationkey"])
            .join(
                duckdb.frame.table("nation"),
    ...
    def test_q08(self, tpch: duckdb.frame.Connection) -> None:
        # The customer's nation is joined first, keeps the plain names, and feeds the region join.
        actual = (
            duckdb.frame.table("part")
            .filter(col("p_type") == "ECONOMY ANODIZED STEEL")
            .join(duckdb.frame.table("lineitem"), on=lambda left, right: left["p_partkey"] == right["l_partkey"])
            .join(duckdb.frame.table("supplier"), on=lambda left, right: left["l_suppkey"] == right["s_suppkey"])
            .join(
                duckdb.frame.table("orders").filter(col("o_orderdate").between(date("1995-01-01"), date("1996-12-31"))),
                on=lambda left, right: left["l_orderkey"] == right["o_orderkey"],
            )
            .join(duckdb.frame.table("customer"), on=lambda left, right: left["o_custkey"] == right["c_custkey"])
            .join(duckdb.frame.table("nation"), on=lambda left, right: left["c_nationkey"] == right["n_nationkey"])
            .join(
                duckdb.frame.table("region").filter(col("r_name") == "AMERICA"),
                on=lambda left, right: left["n_regionkey"] == right["r_regionkey"],
            )
            .join(
                duckdb.frame.table("nation"),
    ...
    def test_q09(self, tpch: duckdb.frame.Connection) -> None:
        actual = (
            duckdb.frame.table("part")
    ...
    def test_q10(self, tpch: duckdb.frame.Connection) -> None:
        orders = duckdb.frame.table("orders").filter(
        ...
        actual = (
            ...
            .join(orders, on=lambda left, right: left["c_custkey"] == right["o_custkey"])
            .join(
                duckdb.frame.table("lineitem").filter(col("l_returnflag") == "R"),
    ...
    def test_q11(self, tpch: duckdb.frame.Connection) -> None:
        german = (
            duckdb.frame.table("partsupp")
            .join(duckdb.frame.table("supplier"), on=lambda left, right: left["ps_suppkey"] == right["s_suppkey"])
            .join(
                duckdb.frame.table("nation").filter(col("n_name") == "GERMANY"),
                on=lambda left, right: left["s_nationkey"] == right["n_nationkey"],
            )
        )
    ...
    def test_q12(self, tpch: duckdb.frame.Connection) -> None:
        urgent = (col("o_orderpriority") == "1-URGENT") | (col("o_orderpriority") == "2-HIGH")
        lineitem = duckdb.frame.table("lineitem").filter(
    ...
    def test_q14(self, tpch: duckdb.frame.Connection) -> None:
        lineitem = duckdb.frame.table("lineitem").filter(
        ...
        actual = lineitem.join(
            duckdb.frame.table("part"), on=lambda left, right: left["l_partkey"] == right["p_partkey"]
        ).aggregate((sql_expr("100.00") * promo / revenue().sum()).alias("promo_revenue"))
        assert actual.rows(tpch) == answer(tpch, "q14")

    def test_q15(self, tpch: duckdb.frame.Connection) -> None:
    ...
    def test_q16(self, tpch: duckdb.frame.Connection) -> None:
        complained = (
            duckdb.frame.table("supplier")
            .filter(col("s_comment").like("%Customer%Complaints%"))
            .select(col("s_suppkey"))
        )
        parts = duckdb.frame.table("part").filter(
    ...
    def test_q18(self, tpch: duckdb.frame.Connection) -> None:
        heavy = (
            duckdb.frame.table("lineitem")
        ...
        actual = (
            duckdb.frame.table("customer")
            .join(
                duckdb.frame.table("orders").filter(col("o_orderkey").isin(heavy)),
    ...
    def test_q19(self, tpch: duckdb.frame.Connection) -> None:
        ...
        actual = (
            duckdb.frame.table("lineitem")
            .join(duckdb.frame.table("part"), on=lambda left, right: left["l_partkey"] == right["p_partkey"])
            .filter(
...
class TestExpressedAfterRewriting:
    ...
    def test_q02(self, tpch: duckdb.frame.Connection) -> None:
        ...
        europe = (
            ...
            .join(duckdb.frame.table("nation"), on=lambda left, right: left["s_nationkey"] == right["n_nationkey"])
            .join(
                duckdb.frame.table("region").filter(col("r_name") == "EUROPE"),
                on=lambda left, right: left["n_regionkey"] == right["r_regionkey"],
            )
        )
    ...
    def test_q04(self, tpch: duckdb.frame.Connection) -> None:
        ...
        actual = (
            ...
            .join(
                duckdb.frame.table("lineitem").filter(col("l_commitdate") < col("l_receiptdate")),
                on=lambda left, right: left["o_orderkey"] == right["l_orderkey"],
                how="semi",
            )
    ...
    def test_q13(self, tpch: duckdb.frame.Connection) -> None:
        # `on` is a join condition, and for a LEFT JOIN filtering the right input first says the same thing.
        orders = duckdb.frame.table("orders").filter(~col("o_comment").like("%special%requests%"))
        per_customer = (
            duckdb.frame.table("customer")
    ...
    def test_q17(self, tpch: duckdb.frame.Connection) -> None:
        ...
        actual = (
            duckdb.frame.table("lineitem")
            .join(
                duckdb.frame.table("part").filter((col("p_brand") == "Brand#23") & (col("p_container") == "MED BOX")),
    ...
    def test_q20(self, tpch: duckdb.frame.Connection) -> None:
        # The correlated sum is rewritten as a join, which drops the rows the original's NULL comparison did.
        forest = duckdb.frame.table("part").filter(col("p_name").like("forest%")).select(col("p_partkey"))
        shipped = (
            duckdb.frame.table("lineitem")
        ...
        actual = (
            ...
            .join(
                duckdb.frame.table("nation").filter(col("n_name") == "CANADA"),
                on=lambda left, right: left["s_nationkey"] == right["n_nationkey"],
            )
    ...
    def test_q21(self, tpch: duckdb.frame.Connection) -> None:
        # EXISTS and NOT EXISTS become a semi join and an anti join, both correlating on more than equality.
        late = duckdb.frame.table("lineitem").filter(col("l_receiptdate") > col("l_commitdate"))

        def another_supplier(left: duckdb.frame.Side, right: duckdb.frame.Side) -> duckdb.frame.Expr:
        ...
        actual = (
            ...
            .join(
                duckdb.frame.table("orders").filter(col("o_orderstatus") == "F"),
                on=lambda left, right: left["l_orderkey"] == right["o_orderkey"],
            )
            .join(
                duckdb.frame.table("nation").filter(col("n_name") == "SAUDI ARABIA"),
                on=lambda left, right: left["s_nationkey"] == right["n_nationkey"],
            )
            .join(duckdb.frame.table("lineitem"), on=another_supplier, how="semi")
    ...
    def test_q22(self, tpch: duckdb.frame.Connection) -> None:
        ...
        actual = (
            ...
            .join(
                duckdb.frame.table("orders"), on=lambda left, right: left["c_custkey"] == right["o_custkey"], how="anti"
            )
